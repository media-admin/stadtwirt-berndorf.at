 ...
 * E-Mail-Versand
 *
 * - Bestätigungsmail an Kunden (standortspezifisches Template)
 * - Kopie-Mail an Filial-E-Mail-Adresse
 * - Platzhalter-Ersetzung
 * - Fallback auf Standard-Template wenn kein individuelles Template gesetzt
 */
...
class MLB_Mail {
    ...
    public static function send_confirmation( int $booking_id ): void {
        ...
        // Kopie an Filiale senden
        if ( $location_email ) {
            $admin_body    = self::admin_notification( $booking_id );
            $admin_subject = '[Neue Buchung] ' . get_the_title( $booking_id );
            wp_mail( $location_email, $admin_subject, self::wrap_html( $admin_body ), $headers );
        }
    }

    // ── Platzhalter ersetzen ──────────────────────────────────────────────────

    private static function replace_placeholders( string $template, int $booking_id ): string {
        $location_id = (int) get_post_meta( $booking_id, 'mlb_booking_location', true );
    ...
    // ── Internes Benachrichtigungs-Template (an Filiale) ─────────────────────
    ...
    private static function admin_notification( int $booking_id ): string {
        ...
        return '
            ...
            <p style="margin-top:24px">
                <a href="' . esc_url( $admin_url ) . '" style="background:#0073aa;color:#fff;padding:10px 18px;text-decoration:none;border-radius:4px">
                    Buchung im Backend ansehen
                </a>
            </p>';
    }

    // ── Kunden-Standard-Template ──────────────────────────────────────────────
    ...
    private static function default_template(): string {
        return '
            ...
            <table cellpadding="8" cellspacing="0" border="0" style="border-collapse:collapse;width:100%">
                ...
                <tr style="background:#f9f9f9"><td><strong>Anmerkungen</strong></td><td>{notes}</td></tr>
            </table>
...
<body>
  <div class="wrap">
    ...
    <div class="footer">' . esc_html( get_bloginfo( 'name' ) ) . ' · ' . esc_html( home_url() ) . '</div>
  </div>
</body>
</html>';
    }
